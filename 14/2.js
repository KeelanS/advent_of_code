...
let lines = new Set(linePoints.map(p => getPointString(p.x, p.y)));
const sand = new Set();

const lowest = max(linePoints.map(x => x.y));
const floor = getLine([createPoint(500 - (lowest + 2), lowest + 2), createPoint(500 + lowest + 2, lowest + 2)]).map(p => getPointString(p.x, p.y))
lines = new Set([...lines, ...floor])
...
while (!isFull) {
    move(start);
    isFull = sand.has(getPointString(start.x, start.y));
    counter++;
}
